"""Construct swarm planners selected by experiment configuration (swarm_lab).

``task_driven`` is the main law; ``bearing`` (v2's role-based projected
bearing law) is kept as the comparison planner; ``constant_velocity`` is
the chassis calibration step utility.
"""
