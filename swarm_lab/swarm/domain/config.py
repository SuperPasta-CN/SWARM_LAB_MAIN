...
class TaskDrivenConfig:
    ...
    Two-phase behavior (``converge_first``): the planner holds the task
    velocity at zero until the mean edge error stays below
    ``phase_converge_eps_m`` for ``phase_converge_hold_s`` seconds
    (or ``phase_converge_timeout_s`` elapses), then switches to the task
    schedule.  The schedule clock starts at that transition.
    """

    k: float = 0.8
    deadband_mps: float = 0.015
    converge_first: bool = True
    phase_converge_eps_m: float = 0.03
    phase_converge_hold_s: float = 2.0
    phase_converge_timeout_s: float = 10.0
...
class BearingControlConfig:
    """Parameters of the projected bearing-only formation law (comparison planner).
    ...
    it (``ki > 0``) and the derivative term adds damping (``kd > 0``).
    """
...
class FirstMateParams:
    """Blending parameters for one first-mate vehicle (comparison planner).

    - ``"weighted"`` (default): ``v = alpha * v_fm + (1 - alpha) * u``
    - ``"additive"``:           ``v = v_fm + u``

    ``velocity=None`` resolves to the captain velocity (Zhao & Zelazo,
    bearing-based formation maneuvering, Proposition 1).
    """
...
@dataclass(frozen=True)
class VehicleConfig:
    """One vehicle.  task_driven has no roles (leave ``role=None``); the
    comparison bearing planner uses ``role`` in ("captain", "first_mate",
    "crew")."""
...
class TopologyConfig:
    ...
    task_driven: ``formation`` maps each vehicle_id to its desired
    world-frame position ``p_i*`` (meters); the controller derives
    ``b_ij = p_j* - p_i*``.  ``edge_weight_matrix`` holds scalar weights
    ``a_ij`` (default 1.0 on adjacent pairs); matrix weights are an
    extension point of the spec.

    bearing (comparison planner): ``bearing_matrix`` holds the desired
    world-frame unit bearing per directed edge (ported from v2), and
    vehicles carry ``role``.
    """

    adjacency_matrix: Sequence[Sequence[int]]
    formation: Optional[Mapping[str, Vector2]] = None
    edge_weight_matrix: Optional[Sequence[Sequence[float]]] = None
    bearing_matrix: Optional[Sequence[Sequence[Sequence[float]]]] = None
...
class ExperimentConfig:
    ...
    # Comparison "bearing" planner fields (ported from v2; only used when
    # algorithm == "bearing").
    ...
    def _validate_topology(self) -> None:
        ...
        if self.algorithm == "bearing":
            bearings = self.topology.bearing_matrix
            if bearings is None or len(bearings) != n or any(len(row) != n for row in bearings):
                raise ValueError("bearing_matrix must be square and match vehicle count")
            self._validate_bearing_roles()
            self._validate_bearing_matrix_consistency()
            return
        formation = self.topology.formation
        missing = [vid for vid in self.vehicle_ids if formation is None or vid not in formation]
        if missing:
    ...
    def _validate_bearing_roles(self) -> None:
        """Role rules for the comparison bearing planner (ported from v2)."""
        ...
        if _speed(self.captain_velocity) > limit:
            raise ValueError("captain_velocity must not exceed command_speed_limit_mps")
        previous_t = 0.0
        for t, velocity in self.captain_velocity_schedule:
            if t < 0.0:
                raise ValueError("captain_velocity_schedule times must be >= 0")
            if t < previous_t:
                raise ValueError("captain_velocity_schedule times must be non-decreasing")
            previous_t = t
