...
@dataclass(frozen=True)
class BearingSample:
    """Desired and measured bearing for one directed topology edge (degrees).

    Only produced by the comparison "bearing" planner (ported from v2).
...
class VelocityCommand:
    ...
    ``role`` is only set by the comparison "bearing" planner
    ("captain"/"first_mate"/"crew"); task_driven leaves it None.
    """
...
class PlannerResult:
    ...
    ``edge_samples`` (meters) are produced by the task_driven planner;
    ``bearing_samples`` (degrees) by the comparison bearing planner.
    """
