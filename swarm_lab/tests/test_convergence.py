...
Uses the genuine ``configs/task_two`` catalog config (and an inline 3-car
...
from configs.task_two import CONFIG
...
class ManeuverDecouplingTests(unittest.TestCase):
    def test_translation_at_w_with_decaying_error(self) -> None:
        # converge_first=False：隔离解耦物理（不经二阶段状态机）
        config = replace(
            CONFIG,
            task_velocity_schedule=(),
            control=TaskDrivenConfig(converge_first=False),
        )
        config.validate()
    ...
    def test_scale_error_corrected_under_active_task(self) -> None:
        # converge_first=False：任务项全程激活，直接考察非任务模态是否泄露
        config = replace(
            CONFIG,
            task_velocity_schedule=(),
            control=TaskDrivenConfig(converge_first=False),
