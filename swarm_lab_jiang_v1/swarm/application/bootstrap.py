...
def _build_vehicle_controller(config: ExperimentConfig):
    if config.execution.mode == "diff":
...
def _run_preflight(config: ExperimentConfig, state_source, topology, assume_yes: bool) -> None:
    """Gate motor unlock on mocap, anchor, and separation checks."""
    ...
    try:
        ...
        # Anchors only matter when leaders ignore the bearing law; in
        # all-bearing mode the formation is free to rotate and translate.
        report = run_preflight_checks(
            state_source.get_snapshot(),
            topology,
            config.preflight,
            check_anchors=config.bearing_control.agent_mode == "leader_velocity",
        )
    ...
    try:
        ...
        for vehicle in config.vehicles:
            ...
            actuators[vehicle.vehicle_id] = GroundVehicleActuator(
                ...
                controller=_build_vehicle_controller(config),
                driver=ChassisSocketDriver(vehicle.address, socket_config),
                min_command=config.execution.wheel_command_min_effective,
            )
        return ControlLoop(
