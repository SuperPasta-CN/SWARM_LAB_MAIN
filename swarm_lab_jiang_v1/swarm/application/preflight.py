...
The checks exist because of a real incident: the leaders were placed with
their baseline rotated ~87 deg from the desired bearing, the followers'
reachable configuration degenerated into an 8 mm slit and the formation
never converged.  Checks 2 and 3 below are hard gates and cannot be skipped
with ``--yes``.
...
class AnchorEdgeFailure:
    """One leader-leader edge whose actual bearing disagrees with the goal."""
...
class PreflightReport:
    ...
    anchor_failures: List[AnchorEdgeFailure] = field(default_factory=list)
...
def run_preflight_checks(
    ...
    """Evaluate every hard check against one mocap snapshot.

    Planar (x, y) bearings are used: the vehicles drive on the floor and the
    desired topology is expressed in the world x/y plane.

    ``check_anchors`` should be False when every agent runs the bearing law
    (``agent_mode="all_bearing"``): the formation is then free to rotate, so
    a rotated leader baseline is no longer a fatal misconfiguration.
    """
    ...
    # 2. anchor consistency on leader-leader edges
    if check_anchors:
        for i, source_id in enumerate(vehicle_ids):
            if not topology.leader_mask[i]:
                continue
            for j, target_id in enumerate(vehicle_ids):
                if not topology.adjacency_matrix[i][j] or not topology.leader_mask[j]:
                    continue
                desired = _normalize2(
                    (
                ...
                if deviation > config.anchor_tolerance_deg:
                    report.anchor_failures.append(
                        AnchorEdgeFailure(
                            source_id=source_id,
                            target_id=target_id,
                            desired=desired,
    ...
    report.ok = (
        not report.waiting_for
        and not report.anchor_failures
        and not report.separation_failures
    )
    return report
...
def print_preflight_report(report: PreflightReport) -> None:
    """Render positions, the per-edge bearing table, and every failure."""
    ...
    for failure in report.anchor_failures:
        print(
            "preflight FAIL | anchor edge %s -> %s: "
