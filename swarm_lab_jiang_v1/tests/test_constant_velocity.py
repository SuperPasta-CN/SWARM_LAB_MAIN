...
class ConstantVelocityPlannerTests(unittest.TestCase):
    ...
    def test_trivial_topology_supports_preflight(self) -> None:
        ...
        self.assertEqual(planner.topology.leader_mask, [False, False])
...
class ChassisStepCatalogTests(unittest.TestCase):
    def test_catalog_config_validates_and_builds(self) -> None:
        from configs import chassis_step

        chassis_step.CONFIG.validate()
        planner = build_swarm_planner(chassis_step.CONFIG)
        self.assertIsInstance(planner, ConstantVelocityPlanner)
