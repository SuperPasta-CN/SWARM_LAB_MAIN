...
class ControlLoop:
    ...
    def __init__(
        ...
        self.converge_eps_rad = converge_eps_rad
    ...
    def _update_convergence(self, mean_error_rad: float, ready: bool, now: float) -> None:
        if not self.stop_on_converge:
            return
        # Only a fully observed formation may accumulate convergence time.
        if ready and mean_error_rad < self.converge_eps_rad:
            if self._below_eps_since is None:
                ...
                self.converged = True
                print(
                    "converged | mean bearing error < %.4f rad for %.1f s, stopping"
                    % (self.converge_eps_rad, self.converge_hold_s)
                )
        else:
